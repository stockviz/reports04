...
aiSummaries1 <- dbGetQuery(mydb, "SELECT * FROM META WHERE UPLOAD_DT = $1 
                           ...
                           and TITLE not LIKE '%startup%' and TITLE not LIKE '%start-up%' and TITLE not LIKE '%start up%' 
                           and SUMMARY not LIKE '%startup%' and SUMMARY not LIKE '%start-up%' and SUMMARY not LIKE '%start up%'
                           order by TS desc", 
...
for(i in 1:nrow(aiSummaries)){
  ...
  cat("\n\n")
  pubTimeStr <- strftime(as.POSIXct(aiSummaries$TS[i]), '%H:%M')
  
  if(!is.na(aiSummaries$SRC[i])){
    cat(paste0("*", 
             aiSummaries$SRC[i], ": ",
             aiSummaries$LINK[i], " ",
             ifelse(is.na(pubTimeStr), "", pubTimeStr), " ",
