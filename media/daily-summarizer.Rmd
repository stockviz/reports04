...
aiSummaries1 <- dbGetQuery(mydb, "SELECT * FROM META WHERE UPLOAD_DT = $1 and SUMMARY is not NULL order by TS desc", 
                          params=list(strftime(asof, "%Y%m%d")))
dbDisconnect(mydb)

mydb <- dbConnect(RSQLite::SQLite(), dbname = newsDbName, flags = RSQLite::SQLITE_RO)
aiSummaries2 <- dbGetQuery(mydb, "SELECT * FROM ARTICLES WHERE PUB_DT = $1 and SUMMARY is not NULL 
